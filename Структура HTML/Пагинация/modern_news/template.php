...
?>
    <div class="modern-page-navigation">
        ...
        ?>
        <!--Слово "страницы" рядом с пагинацией-->
        ...
        if($arResult["bDescPageNumbering"] === true):
            ...
            if ($arResult["NavPageNomer"] < $arResult["NavPageCount"]):
                if($arResult["bSavePage"]):
                    ...
                    <a class="modern-page-previous" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>"><?=GetMessage("nav_prev")?>a</a>
                <?
                else:
                    if ($arResult["NavPageCount"] == ($arResult["NavPageNomer"]+1) ):
                        ?>
                        <a class="modern-page-previous" href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><?=GetMessage("nav_prev")?>b</a>
                    <?
                    else:
                        ?>
                        <a class="modern-page-previous" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>"><?=GetMessage("nav_prev")?>c</a>
                    <?
                    endif;
                endif;
                ...
                if ($arResult["nStartPage"] < $arResult["NavPageCount"]):
                    ...
                    if($arResult["bSavePage"]):
                        ?>
                        <a class="modern-page-first" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=$arResult["NavPageCount"]?>"></a>
                    <?
                    else:
                        ?>
            ...
            if ($arResult["NavPageNomer"] > 1):
                ...
                <?
                    ...
                    if ($arResult["NavPageNomer"] > 2):
                        ?>           <!-- Если больше 2, перемещаемся на страницу назад -->
                        <a class="modern-page-previous" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]-1)?>"><i class="fas fa-chevron-left"></i></a>
                    <?
                    else:
                        ?>
                        <!-- стрелочка слева -->
                        <a class="modern-page-previous" href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>"><i class="fas fa-chevron-left"></i></a>
                    <?
                    endif;
            ...
            if($arResult["NavPageNomer"] < $arResult["NavPageCount"]):
                ...
                ?>
                <!-- стрелочка справа-->
                <a class="modern-page-next" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>"><i class="fas fa-chevron-right"></i></a>
            <?
            endif;
        endif;
